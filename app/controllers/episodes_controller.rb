...
class EpisodesController < ApplicationController
  ...
  def index
    ...
    @episodes = @episodes.select { |episode| episode.creator.id == params[:creator_id].to_i } if params[:creator_id]
  ...
  def user_episodes
    ...
    @episodes = @user_episodes.map(&:episode)
    render :index
  end
  ...
  def get_episode
    ...
    elsif current_user.available_credits.positive?
  ...
  def episode_params
    params.require(:episode).permit(:title, :book_id, :creator_id, :duration, :affiliate_link, :mp3_url,
                                    :preview_url, :notes, :rounded_rating, :state, :published_at)
  end
end
